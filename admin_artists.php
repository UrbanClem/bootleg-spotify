...
if($_POST && isset($_POST['create_artist'])) {
    ...
    $artist->foto_perfil = ''; // Inicializar como vacío

    // Procesar imagen si se subió
    if(isset($_FILES['foto_perfil']) && $_FILES['foto_perfil']['error'] === UPLOAD_ERR_OK) {
        $uploadResult = UploadHelper::uploadArtistImage($_FILES['foto_perfil'], $artist->nombre_artista);
        
        if($uploadResult['success']) {
            $artist->foto_perfil = $uploadResult['file_name'];
        } else {
            $message = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                        Error al subir la imagen: ' . $uploadResult['error'] . '
                        <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                      </div>';
        }
    }

    // Solo crear el artista si no hubo errores con la imagen
    if(empty($message) || strpos($message, 'Error al subir') === false) {
        if($artist->create()) {
            ...
            // Si falló la creación, eliminar la imagen subida
            if(!empty($artist->foto_perfil)) {
                UploadHelper::deleteOldImage($artist->foto_perfil);
            }
        }
    }
}
...
if(isset($_GET['delete_artist'])) {
    ...
    // Primero obtener información del artista para eliminar su imagen
    if($artist->readOne()) {
        $oldImage = $artist->foto_perfil;
        
        if($artist->delete()) {
            // Eliminar la imagen del artista
            if(!empty($oldImage)) {
                UploadHelper::deleteOldImage($oldImage);
            }
...
<head>
    ...
    <title>Gestión de Artistas - Spotify Clone</title>
...
<body style="background: #191414; color: white; min-height: 100vh;">
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="fas fa-microphone me-2"></i>Gestión de Artistas</h2>
            <button class="btn" style="background: #1db954; color: white;" data-bs-toggle="modal" data-bs-target="#createArtistModal">
                            ...
                            <div class="card-body">
                                ...
                                <p class="card-text">
                                    <small class="text-muted">
                                        ...
                                        Registrado: <?php echo date('d/m/Y', strtotime($artistItem['fecha_registro'])); ?>
                                    </small>
                                </p>
                                ...
                                <div class="btn-group w-100">
                                    ...
                                    <a href="edit_artist.php?id=<?php echo $artistItem['id_artista']; ?>" class="btn btn-sm btn-warning">
                                        <i class="fas fa-edit me-1"></i>Editar
                                    </a>
                                    <a href="?delete_artist=<?php echo $artistItem['id_artista']; ?>" class="btn btn-sm btn-danger" 
                                       onclick="return confirm('¿Estás seguro de eliminar este artista? Esta acción no se puede deshacer.')">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <div class="upload-area" onclick="document.getElementById('foto_perfil').click()">
                                    ...
                                    <p class="mb-1">Haz clic para subir una foto</p>
                                    <small class="text-muted">JPEG, PNG, GIF, WebP (Máx. 5MB)</small>
                                </div>
